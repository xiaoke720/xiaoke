...
//var btnCar = document.querySelectorAll('.shoppingBtn');
    ...
    $(btnCar).each(function(index,value){
        ...
        $(this).click(function() {
            ...
            //let num = $("#number").text();
            let totalPrice=parseInt(num)*parseFloat(price);
            ...
            if(cart) {//存在数据,即购物车不为空
                ...
                if(condition){
                    ...
                    //var count=(parseInt(obj.num)*parseFloat(obj.price));
                    $(arr).each(function(index,value){//循环遍历这个数组寻找与新数据匹配的元素
                        //console.log(value);
                        if(obj.dataId === value.dataId && obj.price === value.price && obj.goods === value.goods){
                            //将原新数据的num值合并
                            //obj.num++
                            obj.num = parseInt(obj.num)+parseInt(value.num);
                            obj.totalPrice=(parseInt(obj.num)*parseFloat(obj.price));
                ...
                let arr = [obj];
                window.localStorage.setItem('cart', JSON.stringify(arr));
                //使用localStrong的setItem来往localStrong中存放数据(两个参数:name,value)
            }
        ...
        $.each(arr, function (index, value) {//each循环遍历数组,将数据一一对应添加到#DisplayInformation中并显示到页面中
            ...
            let  result=
                $(
                    ...
                    '<span type="text" class="text-center g-color-gray-dark-v4 two g-font-size-12 number" placeholder="1" aria-describedby="sizing-addon2">'+value.num+'</span>'+
            ...
            let cartCount=$(
                '<span class="g-font-family-bold g-color-gray-dark-v2 g-pl-10">'+value.totalPrice+'</p>'
            )

            $("#cartDropdown").append(numberShow);
            $(".cartCount").find(".jisuan").html(cartCount);
